...
# 7g: простой rate-limit на логин (аудит 🟡-4). In-memory sliding window per ключ
# (email + реальный IP). Малый бизнес — счётчика с окном достаточно, не банк.
# Заметка: лимитер per-process; при >1 реплике лимит умножается — но реплика одна (см. scheduler).
_LOGIN_WINDOW = 60.0   # сек
_LOGIN_MAX = 10        # попыток за окно на ключ
_login_hits: dict[str, deque] = defaultdict(deque)


def _client_ip(request: Request) -> str:
    """Реальный IP клиента: первый хоп X-Forwarded-For (за прокси Railway), иначе peer."""
...
def _rate_limit_login(*keys: str) -> None:
    now = _time.monotonic()
    for key in keys:
        dq = _login_hits[key]
    ...
    # инкремент только если все ключи прошли проверку
    for key in keys:
        _login_hits[key].append(now)
...
async def login(body: LoginBody, request: Request):
    ...
    429 при переборе (rate-limit per email + IP)."""
    email = body.email.strip().lower()
    _rate_limit_login(f"email:{email}", f"ip:{_client_ip(request)}")
