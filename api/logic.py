...
def iter_week_starts(from_d: dt.date, to_d: dt.date):
    """Понедельники недель, ПРИНАДЛЕЖАЩИХ [from_d, to_d] по своему понедельнику (week_start).
    7g (аудит 🟡-5): неделя атрибутируется месяцу своего понедельника, поэтому неделя на
    стыке месяцев (напр. 29 июня–5 июля) считается ровно ОДИН раз — в июне, а не в обоих.
    Первый понедельник берём в пределах диапазона (не заходя в предыдущий месяц)."""
    ws = monday_of(from_d)
    if ws < from_d:
        ws = ws + dt.timedelta(days=7)  # неделя, чей понедельник раньше from_d, принадлежит прошлому периоду
    while ws <= to_d:
        yield ws
